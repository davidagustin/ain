...
static UniValue setlabel(const JSONRPCRequest& request)
{
    ...
    if (IsMineCached(*pwallet, GetScriptForDestination(dest))) {
...
static UniValue getreceivedbylabel(const JSONRPCRequest& request)
{
    ...
    for (const auto& wtx : pwallet->mapWallet.get<ByHash>()) {
        ...
        for (const CTxOut& txout : wtx.tx->vout)
        {
            ...
            if (ExtractDestination(txout.scriptPubKey, address) && IsMineCached(*pwallet, txout.scriptPubKey) && setAddress.count(address)) {
                if (wtx.GetDepthInMainChain(*locked_chain) >= nMinDepth)
                    nAmount += txout.nValue;
            }
        }
...
UniValue ListReceived(interfaces::Chain::Lock& locked_chain, CWallet * const pwallet, const UniValue& params, bool by_label) EXCLUSIVE_LOCKS_REQUIRED(pwallet->cs_wallet)
{
    ...
    for (const auto& wtx : pwallet->mapWallet.get<ByHash>()) {
        ...
        for (const CTxOut& txout : wtx.tx->vout)
        {
            ...
            isminefilter mine = IsMineCached(*pwallet, txout.scriptPubKey);
            if(!(mine & filter))
                continue;
...
static void ListTransactions(interfaces::Chain::Lock& locked_chain, CWallet* const pwallet, const CWalletTx& wtx, int nMinDepth, bool fLong, UniValue& ret, const isminefilter& filter_ismine, const std::string* filter_label) EXCLUSIVE_LOCKS_REQUIRED(pwallet->cs_wallet)
{
    ...
    if (!filter_label)
    {
        for (const COutputEntry& s : listSent)
        {
            ...
            if (involvesWatchonly || (::IsMineCached(*pwallet, GetScriptForDestination(s.destination)) & ISMINE_WATCH_ONLY)) {
    ...
    if (listReceived.size() > 0 && wtx.GetDepthInMainChain(locked_chain) >= nMinDepth)
    {
        for (const COutputEntry& r : listReceived)
        {
            ...
            UniValue entry(UniValue::VOBJ);
            if (involvesWatchonly || (::IsMineCached(*pwallet, GetScriptForDestination(r.destination)) & ISMINE_WATCH_ONLY)) {
                entry.pushKV("involvesWatchonly", true);
            }
...
UniValue getaddressinfo(const JSONRPCRequest& request)
{
    ...
    isminetype mine = IsMineCached(*pwallet, GetScriptForDestination(dest));
